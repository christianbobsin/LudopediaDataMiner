...
cursor.execute("""
SELECT anuncio, validade from jogos where tipo = 'ANUNCIO' and Status = 'Ativo' order by validade asc;
""")
...
for row in cursor.fetchall():
	...
	try:
		page = requests.get('http://www.ludopedia.com.br/anuncio?id_anuncio='+str(row[ID]))
...
#	print str(current) + ' / ' + str(total) + " : " +  "%.2f" % round(progress,2) + "%"
	...
	if  len(jogoNome) :
		...
		formatedRow = "{0:4} | {1:7} | {2:16} | {3:16} | {4:12} | {5:7} | {6}".format( str( jogoCount ).zfill( 4 ),
		                                                  str( row[ID] ),
                                                          row[VALIDADE],
														  status[1],
														  estadoJogo,
														  str(preco),
														  jogoNome[0].encode('utf8') )
		...
		if ( comprador != 'Comprar'):
			...
			cursor.execute("""UPDATE JOGOS SET  STATUS = ?, COMPRADOR = ?, PRECO = ?, VALIDADE = ? where ANUNCIO = ? """,
		 					(status[1], comprador, preco, data, row[ID] ) )
			try:
	  			con.commit()
			except:
        ...
        if ( row[VALIDADE] != str( data ) ):
		    ...
		    novaData = datetime( int(novoAno[2]), int(novoAno[1]),int(novoAno[0]), int(novaHora[0]), int(novaHora[1]))


            cursor.execute("""UPDATE JOGOS SET   PRECO = ?, VALIDADE = ? where ANUNCIO = ? """,
		 					(preco, novaData, row[ID] ) )
            try:
	  			con.commit()
            except:
				print 'Falha no Commit, tentando novamente em 10s.'
		...
		print str( jogoCount ).zfill( 4 ) + ' ' +  str( row[ID] ) + '\t ' + '-------' + '   \t ' + '-------' + '     \t ' + '------' + '\t ' + '---'
